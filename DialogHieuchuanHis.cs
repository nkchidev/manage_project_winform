...
{
    public partial class DialogHieuchuanHis : Office2007Form
    {
        String mCode;
        bool mEditable;
        public DialogHieuchuanHis(String code, bool editable)
        {
            InitializeComponent();
        ...
        private void onLoad(object sender, EventArgs e)
        {
            ...
            this.labelTitle.Text = "Lịch sử hiệu chuẩn: " + mCode;
        }

        void refreshForm()
        {
            DataTable tb = DatabaseUtils.getInstance().getHieuchuanHistoryOfThietbi(mCode);
        ...
        private void buttonHieuchuan_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                try
                {
                    ...
                    if (noidung.Length > 0)
                    {
                        DatabaseUtils.getInstance().insertHieuchuanOfThietbi(mCode, date, noidung,donvi);
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa các hiệu chuẩn đã chọn?", "Cảnh báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.OK)
                {
                    foreach (DataGridViewRow r in dataGridView1.SelectedRows)
                    {
                        ...
                        DatabaseUtils.getInstance().deleteHieuchuan(id);
                    }
